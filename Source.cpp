	...
	while (true) {
		...
		for (int i = 0; i < socketCount; i++) {
			...
			if (sock == listening) {
				...
				clientsInfo[SocketClientsCounter].clientSize = sizeof(clientsInfo[SocketClientsCounter].client);

				ZeroMemory(clientsInfo[SocketClientsCounter].host, NI_MAXHOST);
				ZeroMemory(clientsInfo[SocketClientsCounter].service, NI_MAXSERV);

				//accepts incoming connection
				SOCKET client = accept(listening, (sockaddr*)&clientsInfo[SocketClientsCounter].client, &clientsInfo[SocketClientsCounter].clientSize);
				if (client == SOCKET_ERROR) {
					cerr << "Can't accept socket client incomming request.." << endl;
				}

				FD_SET(client, &master);

				if (getnameinfo((sockaddr*)&clientsInfo[SocketClientsCounter].client, sizeof(clientsInfo[SocketClientsCounter].client), clientsInfo[SocketClientsCounter].host, NI_MAXHOST, clientsInfo[SocketClientsCounter].service, NI_MAXSERV, 0) == 0)
				{
					cout << clientsInfo[SocketClientsCounter].host << " connected on port " << clientsInfo[SocketClientsCounter].service << endl;
				}
				else
				{
					inet_ntop(AF_INET, &clientsInfo[SocketClientsCounter].client.sin_addr, clientsInfo[SocketClientsCounter].host, NI_MAXHOST);
					cout << clientsInfo[SocketClientsCounter].host << " connected on port " << ntohs(clientsInfo[SocketClientsCounter].client.sin_port) << endl;
				}

				SocketClientsCounter++;
			}
			else {
				char buf[4096];
				ZeroMemory(buf, 4096);
					...
					if (!strcmp(buf, "1") || !strcmp(buf, "2") || !strcmp(buf, "3")) {
						...
						for (int i = 0; i < master.fd_count; i++) {
							...
							if (outSock != listening && outSock == sock) {
								...
								cout << "Mailing was cancelled by client " << clientsInfo[i - 1].host << " on port : " << clientsInfo[i - 1].service << endl;
